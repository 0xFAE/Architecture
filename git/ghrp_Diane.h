...
Static RRIPclass GHRP : public ReplPolicy { 
	...
	public: 
		...
		block VictimBlock(struct block set[]);
};
...
GHRP::Access(int _pc) {
	...
	auto set = CalcSet(_pc, cache);  //TODO define calc set function
	auto tag = CalcTag(_pc, cache); //TODO define function and figure 
	bool isMissed = isTagMatch(_pc, cache); //TODO define function
	if(isMissed) {
		auto bypass = MajorityVote(cntrsNew, bypassThresh); //TODO define threshold
		if(!bypass) {
			auto block = VictimBlock(set);
			ComputeIndices(block.signature, indices);
			bool isDead = true;
		...
		auto block = MatchedBlock(set, tag);          //TODO define MatchedBlock function
		ComputeIndices(block.signature, indices);
		bool isDead = false;
		UpdatePredTables(indices, isDead);
		auto pred = MajorityVote(cntrsNew, deadThresh);
		block.dead = pred;
	}
	block.signature = sign;
	UpdatePathHist();
	UpdateLRUStackPosition(); //TODO define function
}
...
GHRP::GetCounters(int _PredTables[][], int _ indices, int _counters[]){ 
	for(int t=1; t < m_numPredTables; t++){ 
		_counters[t] = _predTables[indices[t]][t]; 
	} 
	return  _counters[m_numPredTables];
}
...
/*-------------- Algorithm 5 -------------------------*/
 block VictimBlock(struct block _set[]){
       for(int i=1; i <  ;i++){
            current_block = _set[i]; 
            if (block.isDead) {
